...
First, inspected for any N/As in the dataser, using the visna function. 

```{r missing, echo=TRUE}
visna(theoffice_df, sort = "b")
```

We found that there were none of these values in the entire data (nice!), but this doesn't mean that the data was usable straight away. We glimpsed into it to better understand what was going on with the different columns.
