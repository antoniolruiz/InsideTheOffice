...
```{r cast, echo=TRUE}
##Filtering for top 20 characters only
...
ggplot(dialogues_by_char, aes(x=fct_reorder(dialogues_by_char$speaker, Freq, .desc = TRUE)), stat = "count") + 
  geom_bar() + 
  coord_flip() + 
...
ggplot(word_length_df, aes(x = reorder(speaker, max_word), y = max_word)) + 
  geom_point() + 
  coord_flip() + 
...
12) Who is the character that says the longest lines?

```{r longest_lines, echo=TRUE}
### Changed dialogues_df; leaving it here in the meanwhile
...
ggplot(dialogue_length_df, aes(x = reorder(speaker, max_dialogue), y = max_dialogue)) + 
  geom_point() + 
  coord_flip() + 
  labs(title = "Who speaks the longest lines?", x = "Character", y="Average characters per line of speech") + 
  lims(y=c(20,35)) + 
...
ggplot(words_by_char, aes(x = reorder(speaker, -Freq), y = Freq)) + 
  geom_point() + 
  coord_flip() + 
  labs(title = "Who speaks the most words?", x = "Character", y="Total number of words throughout the show") + 
  lims(y=c(20,35)) + 
  scale_fill_brewer(palette="Set1")
...
7) What are the groups of characters that share the most screen time?
##By the way the table is constructed, we can see individual characters, as well as groups. Should we change this?
...
### Is this information relevant enough to include in previous charts?
