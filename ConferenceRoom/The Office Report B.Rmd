...
This was great progress to start our analysis, with it, we were capable of focusing on the actual dialogues, which is the main objective of this project. We initiated the process by removing the wrylies, which allowed us to focus on dialogues rather than instructions. We also dropped punctuation symbols and removed capitalization from the dataset. Finally, we tied common expressions throughout the series, joining each word by an underscore, in order to avoid dropping the stopwords included in these expressions later in the project.
...
### We need to redefine colors for this chart. 
```{r TWSS, echo=TRUE}
